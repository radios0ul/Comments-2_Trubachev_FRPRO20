...
class Render {
    constructor(user, factory, commentBase) {
        this.commentsBlock = (document.querySelector(".comments__block"));
        this.commentInput = (document.querySelector(".comm-input"));
        this.postCommentBtn = document.querySelector(".comm__form-send-btn");
        this.user = user;
    ...
    renderCommentBase() {
        ...
        let allComments = this.commentBase.getAllComments();
        allComments.forEach((comment) => {
            this.renderComment(comment, false);
        });
    }
    renderComment(comment, favOnly) {
        //данные комментария
        let commentText = comment.getCommentText();
        let user = comment.getUser();
        let currentUser = new user_1.User();
        let date = comment.getCommentDate();
        let commentID = comment.getCommentID();
        let rating = comment.getRating();
        let isFav = comment.getIsFavorite();
        ...
        const favBtn = newCommentHtml.querySelector(".fav__btn");
        if (!favBtn)
            return;
        const favDiv1 = newCommentHtml.querySelector(".added");
        const favDiv2 = newCommentHtml.querySelector(".not-added");
        if (!favDiv1)
            return;
        if (!favDiv2)
            return;
        ...
        //рейтинг коммента
        const plusBtn = newCommentHtml.querySelector(".plus__btn");
        const minusBtn = newCommentHtml.querySelector(".minus__btn");
        const ratingDiv = newCommentHtml.querySelector(".karma-counter");
        ...
        plusBtn.addEventListener("click", () => {
            let currentRatingPlus = comment.getRating();
        ...
        const replyBtn = newCommentHtml.querySelector(".comment__answer-btn");
        const replyInputBlock = newCommentHtml.querySelector(".reply-input-block");
        if (!replyBtn)
            return;
        if (!replyInputBlock)
            return;
        ...
        let replies = comment.getReplies();
        if (favOnly == true) {
            // если отрисовка идет по команде "показать избранное"
            replies.forEach((element) => {
                let isF = element.getIsFavorite();
                if (isF == true) {
        ...
        const replyInput = newCommentHtml.querySelector(".reply-input");
        const replyPostBtn = newCommentHtml.querySelector(".reply-post-btn");
        if (!replyInput)
            return;
        if (!replyPostBtn)
            return;
        replyPostBtn.addEventListener("click", () => {
            const userTextOfReply = replyInput.value;
            if (userTextOfReply === "") {
                alert("Nothing to post!");
            }
            else {
                let newReply = this.factory.createReply(userTextOfReply, this.user, commentID, comment, this.commentBase, 0, false, new Date());
                this.renderReply(newReply, newCommentHtml);
                replyInput.value = "";
            }
        });
    }
    renderReply(comment, newCommentHtml) {
        //данные ответа
        let commentText = comment.getCommentText();
        let user = comment.getUser();
        let date = comment.getCommentDate();
        let commentID = comment.getCommentID();
        let parentID = comment.getParentID();
        let replyRating = comment.getRating();
        let isFavorite = comment.getIsFavorite();
        ...
        let parentComment = (this.commentBase.getCommentById(parentID)); // родительский коммент
        let newReplyHtml = document.createElement("div");
...
`;
        ...
        const favDiv1 = newReplyHtml.querySelector(".added");
        const favDiv2 = newReplyHtml.querySelector(".not-added");
        if (!favDiv1)
            return;
        if (!favDiv2)
            return;
        ...
        const favBtn = newReplyHtml.querySelector(".fav__btn");
        if (!favBtn)
            return;
        favBtn.addEventListener("click", () => {
            comment.toggleFavorite(user);
            ...
            let parentFav = parentComment.getReplies();
            let flag = false;
        ...
        //рейтинг ответа
        const plusBtn = newReplyHtml.querySelector(".plus__btn");
        const minusBtn = newReplyHtml.querySelector(".minus__btn");
        const ratingDiv = newReplyHtml.querySelector(".karma-counter");
        let initRating = comment.getRating();
        if (!ratingDiv)
            return;
        if (!plusBtn)
    ...
    buttonListener() {
        ...
        const postCommentBtn = this.postCommentBtn;
        const commentCounter = (document.querySelector(".comm__h-count"));
        if (!commentInput)
            return;
        if (!postCommentBtn)
            return;
        postCommentBtn.addEventListener("click", () => {
            const userTextOfComment = commentInput.value;
            if (userTextOfComment === "") {
                alert("Nothing to post!");
            }
            else {
                let newComment = this.factory.createComment(userTextOfComment, this.user, 0, this.commentBase, 0, false, new Date());
    ...
    favListener() {
        ...
        let favPressed = false;
        const favOnImg = document.querySelector(".fav-on");
        const favOffImg = document.querySelector(".fav-off");
        const showFavBtn = (document.querySelector(".comm__favourities"));
    ...
    clearAll() {
        ...
        const allCommentsBlock = (document.querySelector(".comments__block"));
    ...
    filterMenuListener() {
        const filterBtn = (document.querySelector(".comm__btn"));
        const filtermenu = (document.getElementById("filtermenu"));
        filterBtn.addEventListener("click", () => {
            filtermenu.classList.toggle("hidden");
        });
        const menuLegend = (document.querySelector(".comm__filter-text"));
        const byDate = (document.getElementById("by-date"));
        const byRating = (document.getElementById("by-rating"));
        const byActuality = (document.getElementById("by-actuality"));
        const byAnswers = (document.getElementById("by-answers"));
        const byDateG = (document.querySelector(".by-date-g"));
        const byRatingG = (document.querySelector(".by-rating-g"));
        const byActualityG = (document.querySelector(".by-actuality-g"));
        const byAnswersG = (document.querySelector(".by-answers-g"));
        const sortArrow = (document.querySelector(".sort"));
        const arrowUp = (document.querySelector(".arrow-up"));
        const arrowDwn = (document.querySelector(".arrow-down"));
    ...
    textLengthChecker() {
        const maxSymbolQty = 400; // ограничение по кол-ву символов
        const infoMsg = (document.querySelector(".comm__form-symbols-counter"));
        const replyBtn = document.querySelector(".reply-post-btn");
        let warnMsg = document.querySelector(".warning__message");
        let warnMsgReply = document.querySelector(".reply-warning__message");
        ...
        let replyInfoMsg = (document.querySelector(".comm__form-symbols-reply"));
        if (!replyInput)
            return;
    ...
    textareaResize() {
        const textarea = (document.querySelector(".comm-input"));
        const replyArea = document.querySelector(".reply-input");
        ...
        const postBtn = document.querySelector(".comm__form-send-btn");
        const replyBtn = document.querySelector(".reply-post-btn");
        if (!postBtn)
            return;
