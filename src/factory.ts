...
export class Factory {
  ...
  saveChangedBase(commentBase: CommentBase) {
    localStorage.setItem("savedComments", JSON.stringify(commentBase));
  }
  ...
  readInitComments(commentBase: CommentBase) {
    let inCom: string = `{"commentBase":
[{"commentText":"Самое обидное когда сценарий по сути есть - в виде книг, где нет сюжетных дыр, всё логично, стройное повествование и достаточно взять и экранизировать оригинал как это было в первых фильмах с минимальным количеством отсебятины и зритель с восторгом примет любой такой фильм и сериал, однако вместо этого 'Кольца власти' просто позаимствовали имена из оригинала, куски истории, мало связанные между собой и выдали очередной среднячковый сериал на один раз в лучшем случае.",
   ...
   "replies":[
    ...
    let savedCommentsJson: string | null =
  ...
  fillCommentBase() {
    let commentBase = new CommentBase();
  ...
  readCommentsFromStorage(commentBase: CommentBase) {
    let savedCommentsJson: string | null =
      localStorage.getItem("savedComments");
    ...
    if (!!savedCommentsJson) {
      ...
      if (!!savedComments) {
        for (let i = 0; i <= savedComments.commentBase.length - 1; i++) {
          ...
          let currentCommentDate: Date = new Date(
            Date.parse(currentComment.commentDate)
          );
          ...
          let newComment = new Comment(
            ...
            currentComment.commentRating,
            currentComment.isFavorite,
            currentCommentDate
          );
          ...
          for (let j = 0; j <= currentComment.replies.length - 1; j++) {
            ...
            let currentReplyDate: Date = new Date(
              Date.parse(currentReply.commentDate)
            );

            let newReply = new Comment(
              currentReply.commentText,
              currentReply.commentId,
              currentReply.user,
              currentReply.parentId,
              currentReply.commentRating,
              currentReply.isFavorite,
              currentReplyDate
            );
